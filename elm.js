...
var $author$project$Main$updatePlayer = function (model) {
	...
	return A2($author$project$Main$playerOnPlatforms, player, platforms) ? _Utils_update(
		player,
		...
		switch (msg.$) {
			case 'OnAnimationFrame':
				...
				var updatedModel = _Utils_update(
					model,
					{
						alive: (!updatedPlayer.y) ? false : true,
						platforms: $author$project$Main$movePlatforms(model.platforms),
			...
			case 'KeyUp':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							player: $author$project$Main$stopXMotion(model.player)
						}),
					$elm$core$Platform$Cmd$none);
			default:
		...
		A2(
			...
			function (platform) {
				return A3(
					...
					_Utils_Tuple2(platform.x, platform.y),
					platform.width,
